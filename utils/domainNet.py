...
import utils.data_list_dnet
from utils.config import FLAGS
from utils.data_list_dnet import ImageList
import numpy as np
import numbers
...
def get_domainNet(source_path, target_path, batch_size):
    ...
    dsets_val = ImageList(open(FLAGS.target_test_path).readlines(), transform=data_transform_test)

#     class_sample_count = [0.0] * 345
# 
...
#     sampler = torch.utils.data.sampler.WeightedRandomSampler(class_sample_weights, len(dsets_source)) # Uniform Sampler across classes.
    ...
    if FLAGS.is_cl_bln:
        source_train_loader = util_data.DataLoader(dsets_source, batch_size=FLAGS.s_bs, shuffle=False, num_workers=0,  drop_last=True, sampler=sampler)
    else:
        source_train_loader = util_data.DataLoader(dsets_source, batch_size=FLAGS.s_bs, shuffle=True, num_workers=0, drop_last=True)
    target_train_loader = util_data.DataLoader(dsets_target, batch_size=FLAGS.t_bs, shuffle=True, num_workers=0, drop_last=True)
    target_val_loader = util_data.DataLoader(dsets_val, batch_size=batch_size, shuffle=False, num_workers=0, drop_last=True)
 
    print('Found {} Mini-batches for DomainNet (Source-Train) -- '.format(
        str(len(source_train_loader))))
    print('Found {} Mini-batches for DomainNet (Target-Train) -- '.format(
        str(len(target_train_loader))))
    print('Found {} Mini-batches for DomainNet (Target-Val) -- '.format(
